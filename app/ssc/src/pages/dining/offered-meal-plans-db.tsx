...
export default function OfferedMealPlansPage() {
  ...
  const [selectedTypes, setSelectedTypes] = useState<Record<string, "Year" | "Semester">>({});
  const [mealPlans, setMealPlans] = useState<MealPlanOption[]>([]);

  useEffect(() => {
    const fetchMealPlans = async () => {
      try {
        const response = await fetch(`/api/meal_plans?userId=${currentUserId}`);
        if (response.ok) {
          const data = await response.json();
          setMealPlans(data);
          const initialTypes: Record<string, "Year" | "Semester"> = {};
          data.forEach((plan: MealPlanOption) => {
  ...
  const handleCheckout = (plan: MealPlanOption) => {
    ...
    router.push(
      `/checkout?price=${price}&type=meal-plan&planId=${plan.id}&planType=${selectedTypes[plan.id] || "Year"}&from=dining/dining-page`
    );
  };
